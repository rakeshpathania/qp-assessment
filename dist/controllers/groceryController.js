...
const addGroceryItem = async (req, res, next) => {
    try {
        ...
        res.status(STATUS_CODES.CREATED).json(SUCCESS_CREATED_STATUS("Grocery item added successfully", newItem));
    }
    catch (error) {
        next(error);
    }
};
const getGroceryItems = async (req, res, next) => {
    try {
        const { skip, take } = req.query;
        const safeSkip = parseInt(skip, 10) || 0;
        const safeTake = parseInt(take, 10) || 10;
...
const removeGroceryItem = async (req, res, next) => {
    try {
...
const updateInventoryLevel = async (req, res, next) => {
    try {
        const itemId = parseInt(req.params.id, 10);
        const { quantityInStock } = req.body;
        const updatedItem = await groceryItemService.updateInventory(itemId, Math.max(0, Number(quantityInStock)));
        res.status(STATUS_CODES.OK).json(SUCCESS_STATUS("Inventory level updated successfully", updatedItem));
    }
    catch (error) {
        next(error);
    }
};
const getAvailableGroceryItems = async (req, res, next) => {
    try {
